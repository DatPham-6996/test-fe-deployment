...
import { FlipExternalEvent } from '@/lib/__generated__/graphql';
...
type ExternalEventTimelineProps = {
  events: FlipExternalEvent[];
  loading?: boolean;
  onLoadMore?: () => void;
...
export default function ExternalEventTimeline({
  events,
  loading,
  onLoadMore,
  ...
  return (
    <div className="flex justify-center">
      ...
      {!loading && showEmptyState && !events.length ? (
      ...
      {!loading && events.length > 0 && (
        <div className="flex flex-col items-center w-full">
          {events.map((event: FlipExternalEvent, index: number) => {
            // Filter out all events from Flip and user not belong to Flip Team
            if (event?.organization?.id === FLIP_ORG_ID && !isFlipTest) return;
