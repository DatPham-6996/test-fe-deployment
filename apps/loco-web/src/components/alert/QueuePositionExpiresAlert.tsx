...
type QueuePositionExpiresAlertProps = {
  open: boolean;
  onClick: (e: React.MouseEvent<HTMLButtonElement>) => void;
};

export function QueuePositionExpiresAlert({ open, onClick }: QueuePositionExpiresAlertProps) {
      ...
      <AlertDialogContent className="py-14 px-8 max-w-[400px]">
        <div className="flex flex-col justify-center items-center gap-5">
          ...
          <AlertDialogTitle className="text-xl font-semibold my-1">
            {formatMessage({ id: 'waitingRoom.queuePositionExpires' })}
          </AlertDialogTitle>
          <AlertDialogDescription className="my-1 text-center">
            {formatMessage({ id: 'waitingRoom.queuePositionExpiresDes' })}
          </AlertDialogDescription>
        </div>

        <div className="flex justify-center mt-1">
          <Button size="lg" onClick={onClick}>
            {formatMessage({ id: 'waitingRoom.goBack' })}
          </Button>
        </div>
      </AlertDialogContent>
    </AlertDialog>
