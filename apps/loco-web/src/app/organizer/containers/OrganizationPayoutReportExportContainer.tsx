...
export default function OrganizationPayoutReportExportContainer() {
  ...
  const [requestPayoutReportMutation, { loading: requestPayoutReportLoading }] =
    useRequestOrganizationPayoutReportMutation({
      variables: {
        input: {
  ...
  const onInitialize = useCallback(async () => {
    if (requestPayoutReportLoading) {
      return;
    }
    const { data } = await requestPayoutReportMutation();
    const reportId = data?.requestOrganizationPayoutReport.reportId;
    if (!reportId) {
      throw new Error('Report ID not found.');
    }
    setReportId(reportId);
  }, [requestPayoutReportLoading, requestPayoutReportMutation]);
    ...
    return (
      <>
        <h1 className="text-2xl font-bold mb-2 sm:mb-0">
          {formatMessage({ id: 'payout.export.organizationPayoutTitle' })}
        </h1>
        <p className="text-base text-foreground mb-4 sm:mb-0">
          <span>{formatMessage({ id: 'payout.export.organizationPayoutDescription' })}</span>
        </p>
        <DateRangePicker
          ...
          label={formatMessage({ id: 'payout.export.organizationPayoutSelectTimeRange' })}
        />
        <Button
